    ...
    @@foreach($locales as $locale)
        <div class="col"@@if(!$loop->first) v-show="isFormLocalized && currentLocale == '@{{ $locale }}'" v-cloak @@endif>
            <div class="form-group row" :class="{'has-danger': errors.has('@{{ $locale }}_{{ $col['name'] }}'), 'has-success': this.fields.@{{ $locale }}_{{ $col['name'] }} && this.fields.@{{ $locale }}_{{ $col['name'] }}.valid }">
                <label for="@{{ $locale }}_{{ $col['name'] }}" class="col-md-3 col-form-label text-md-right">{{'{{'}} trans('admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}</label>
                <div class="col-md-9" :class="{'col-xl-8': !isFormLocalized }">
                    ...
                    @else<input type="text" v-model="form.{{ $col['name'] }}.@{{ $locale }}" @@if($loop->first) v-validate="'{!! implode('|', $col['frontendRules']) !!}'" @@endif class="form-control" :class="{'form-control-danger': errors.has('@{{ $locale }}_{{ $col['name'] }}'), 'form-control-success': this.fields.@{{ $locale }}_{{ $col['name'] }} && this.fields.@{{ $locale }}_{{ $col['name'] }}.valid }" id="@{{ $locale }}_{{ $col['name'] }}" name="@{{ $locale }}_{{ $col['name'] }}" placeholder="{{'{{'}} trans('admin.{{ $modelLangFormat }}.columns.{{ $col['name'] }}') }}">
