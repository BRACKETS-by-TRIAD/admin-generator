...
@foreach($columns as $col)@if($col['name'] == 'password')<div class="form-group row" :class="{'has-danger': errors.has('{{ $col['name'] }}'), 'has-success': this.fields.{{ $col['name'] }} && this.fields.{{ $col['name'] }}.valid }">
    <label for="{{ $col['name'] }}" class="col-md-3 col-form-label text-md-right">{{'{{'}} trans('admin.{{ $modelDotNotation }}.columns.{{ $col['name'] }}') }}</label>
    <div class="col-md-9 col-xl-8">
        <input type="password" v-model="form.{{ $col['name'] }}" v-validate="'{{ implode('|', $col['frontendRules']) }}'" class="form-control" :class="{'form-control-danger': errors.has('{{ $col['name'] }}'), 'form-control-success': this.fields.{{ $col['name'] }} && this.fields.{{ $col['name'] }}.valid}" id="{{ $col['name'] }}" name="{{ $col['name'] }}" placeholder="{{'{{'}} trans('admin.{{ $modelDotNotation }}.columns.{{ $col['name'] }}') }}">
...
<div class="form-group row" :class="{'has-danger': errors.has('{{ $col['name'] }}_confirmation'), 'has-success': this.fields.{{ $col['name'] }}_confirmation && this.fields.{{ $col['name'] }}_confirmation.valid }">
    <label for="{{ $col['name'] }}_confirmation" class="col-md-3 col-form-label text-md-right">{{'{{'}} trans('admin.{{ $modelDotNotation }}.columns.{{ $col['name'] }}_repeat') }}</label>
    <div class="col-md-9 col-xl-8">
        <input type="password" v-model="form.{{ $col['name'] }}_confirmation" v-validate="'{{ implode('|', $col['frontendRules']) }}'" class="form-control" :class="{'form-control-danger': errors.has('{{ $col['name'] }}_confirmation'), 'form-control-success': this.fields.{{ $col['name'] }}_confirmation && this.fields.{{ $col['name'] }}_confirmation.valid}" id="{{ $col['name'] }}_confirmation" name="{{ $col['name'] }}_confirmation" placeholder="{{'{{'}} trans('admin.{{ $modelDotNotation }}.columns.{{ $col['name'] }}') }}">
...
@elseif($col['type'] == 'boolean')<div class="form-check row" :class="{'has-danger': errors.has('{{ $col['name'] }}'), 'has-success': this.fields.{{ $col['name'] }} && this.fields.{{ $col['name'] }}.valid }">
    <div class="col-md-9 col-xl-8 offset-md-3">
        <label class="form-check-label">
            ...
            {{'{{'}} trans('admin.{{ $modelDotNotation }}.columns.{{ $col['name'] }}') }}
            <div v-if="errors.has('{{ $col['name'] }}')" class="form-control-feedback" v-cloak>{{'@{{'}} errors.first('{{ $col['name'] }}') }}</div>
        </label>
    </div>
</div>
    ...
    @@foreach($locales as $locale)
        <div class="col"@@if(!$loop->first) v-show="isFormLocalized && currentLocale == '@{{ $locale }}'" v-cloak @@endif>
            <div class="form-group row" :class="{'has-danger': errors.has('@{{ $locale }}_{{ $col['name'] }}'), 'has-success': this.fields.@{{ $locale }}_{{ $col['name'] }} && this.fields.@{{ $locale }}_{{ $col['name'] }}.valid }">
                <label for="@{{ $locale }}_{{ $col['name'] }}" class="col-md-3 col-form-label text-md-right">{{'{{'}} trans('admin.{{ $modelDotNotation }}.columns.{{ $col['name'] }}') }}</label>
                <div class="col-md-9" :class="{'col-xl-8': !isFormLocalized }">
                    ...
                    @else<input type="text" v-model="form.{{ $col['name'] }}.@{{ $locale }}" @@if($loop->first) v-validate="'{!! implode('|', $col['frontendRules']) !!}'" @@endif class="form-control" :class="{'form-control-danger': errors.has('@{{ $locale }}_{{ $col['name'] }}'), 'form-control-success': this.fields.@{{ $locale }}_{{ $col['name'] }} && this.fields.@{{ $locale }}_{{ $col['name'] }}.valid }" id="@{{ $locale }}_{{ $col['name'] }}" name="@{{ $locale }}_{{ $col['name'] }}" placeholder="{{'{{'}} trans('admin.{{ $modelDotNotation }}.columns.{{ $col['name'] }}') }}">
...
@else<div class="form-group row" :class="{'has-danger': errors.has('{{ $col['name'] }}'), 'has-success': this.fields.{{ $col['name'] }} && this.fields.{{ $col['name'] }}.valid }">
    <label for="{{ $col['name'] }}" class="col-md-3 col-form-label text-md-right">{{'{{'}} trans('admin.{{ $modelDotNotation }}.columns.{{ $col['name'] }}') }}</label>
    <div class="col-md-9 col-xl-8">
        <input type="text" v-model="form.{{ $col['name'] }}" v-validate="'{{ implode('|', $col['frontendRules']) }}'" class="form-control" :class="{'form-control-danger': errors.has('{{ $col['name'] }}'), 'form-control-success': this.fields.{{ $col['name'] }} && this.fields.{{ $col['name'] }}.valid}" id="{{ $col['name'] }}" name="{{ $col['name'] }}" placeholder="{{'{{'}} trans('admin.{{ $modelDotNotation }}.columns.{{ $col['name'] }}') }}">
        <div v-if="errors.has('{{ $col['name'] }}')" class="form-control-feedback" v-cloak>{{'@{{'}} errors.first('{{ $col['name'] }}') }}</div>
    </div>
</div>
@endif
